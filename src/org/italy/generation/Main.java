	...
	public static void main(String[] args) {
		...
		do {	//Inizio la sezione in cui il cliente può fare gli acquisti
			...
			for (i=0; i<nProdotti; i++) { //inizio la ricerca del prodotto inserito
				if (nome[i].contains(prodottoDaAcquistare)) {	//controllo se la stringa inserita è contenuta in uno dei prodotti in negozio
					//adesso voglio controllare che il prodotto trovato corrisponda a quello richiesto dall'utente
					System.out.println(nome[i].substring(0, 1).toUpperCase()+nome[i].substring(1)+" è il prodotto che stavi cercando?");
					...
					if (risposta.substring(0, 1).equals("s")) { //il prodotto trovato corrisponde a quello voluto dal cliente
						System.out.println("Ok! Sono rimasti "+qnt[i]+" elementi in negozio. Quanto ne vuoi acquistare?");
						qntDaAcquistare= sc.nextInt();
						sc.nextLine();
						trovato=true;
						if (qntDaAcquistare>qnt[i]) {
							System.out.println("Il prodotto richiesto non è presente in negozio nella quantità richiesta.");
						}else {
							importoTotale=importoTotale+(prezzo[i]*qntDaAcquistare);
							System.out.println("Prodotto aggiunto al carrello. L'importo parziale è: "+(prezzo[i]*qntDaAcquistare)+" euro, mentre il totale è: "+importoTotale+" euro.");
